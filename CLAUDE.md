...
- **Planets** (`world::planet_list`, salt-cached): 6 huge landable bodies (90–240m radius) strung along `world::SYSTEM_AXIS` between the belt (home, at the system's edge) and the star. Same voxel pipeline as asteroids — land, walk, mine, blast. Tier climbs toward the star (4 → 12): the endgame lives deep in the kill zone and gets eaten mid-day, so reaching it is a race. Each planet gets one always-visible ico(4) impostor (never proximity-hidden — streamed chunks just draw over it locally); belt cells inside a planet's reach are suppressed.
...
- World: an unbounded 3D asteroid field, chunked 16³ in all directions. A deterministic hash gives each 56m cell at most one asteroid — **very sparse (1.8% of cells) but individual**: per-seed ellipsoid stretch, displacement amplitude/frequency, and a **species** (8 palettes/names driving rock color; ore color stays tier-based so progression reads). A starter rock is guaranteed within 2 cells of home each day. The home rock sits at the origin, fixed across days. Planets live outside the cell registry (`planet_list`, checked directly in `asteroids_overlapping`). Worldgen is a pure function of (voxel coords, world salt) — `world::set_world_salt` is the one mutable input, bumped per day; tests assume salt 0 and must never change it (process-global, tests run in parallel).
